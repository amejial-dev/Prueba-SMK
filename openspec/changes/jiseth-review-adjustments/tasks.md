...
## 9. Migraciones de Sequelize en directorio propio

- [x] 9.1 En `backend/package.json`: agregar `sequelize-cli` (`^6.6.5`) a `devDependencies` (`npm install --save-dev sequelize-cli`). Agregar los scripts `"migrate": "sequelize-cli db:migrate"`, `"migrate:undo": "sequelize-cli db:migrate:undo"`, `"migrate:undo:all": "sequelize-cli db:migrate:undo:all"`, `"migration:generate": "sequelize-cli migration:generate"`. Modificar `"start"` a `"npm run migrate && node src/index.js"` y `"dev"` a `"npm run migrate && nodemon src/index.js"`.
- [x] 9.2 Crear `backend/.sequelizerc` fijando: `config` → `src/config/database.cli.js`, `migrations-path` → `src/database/migrations`, `seeders-path` → `src/database/seeders`, `models-path` → `src/models` (los modelos de runtime no se mueven; solo se separa el directorio de migraciones).
- [x] 9.3 Crear `backend/src/config/database.cli.js`: exporta `{ development, test, production }`, cada uno con `{ username: DB_USER, password: DB_PASSWORD, database: DB_NAME, host: DB_HOST, port: DB_PORT || 5432, dialect: 'postgres', logging: false }` leídos con `require('dotenv').config()`. No reemplaza a `backend/src/config/database.js` (que sigue siendo la conexión de runtime que usan los modelos).
- [x] 9.4 Crear en `backend/src/database/migrations/` tres migraciones, en este orden (por dependencias de FK), reproduciendo exactamente el esquema que hoy generaba `sync()`:
  - `<timestamp>-create-users.js`: tabla `users` (`id`, `nombre` único, `passwordHash`, `rol` enum `user`/`admin`, `createdAt`, `updatedAt`).
  - `<timestamp+1>-create-documents.js`: tabla `documents` (`id`, `originalName`, `fileName`, `filePath`, `recordCount`, `userId` FK a `users.id` con `onDelete: 'CASCADE'`, `createdAt`, `updatedAt`, `deletedAt` nullable — esta última por `paranoid: true` de la tarea 1.1).
  - `<timestamp+2>-create-document-rows.js`: tabla `document_rows` (`id`, `correo`, `nombre`, `telefono`, `ciudad`, `notas` nullable, `documentId` FK a `documents.id` con `onDelete: 'CASCADE'`, `createdAt`, `updatedAt`).
  Cada migración implementa `up` (crear tabla) y `down` (`dropTable`; en `create-users` además `DROP TYPE IF EXISTS "enum_users_rol"` para dejar limpio el tipo ENUM de Postgres que crea Sequelize).
- [x] 9.5 En `backend/src/index.js`: eliminar la llamada a `sequelize.sync()` (y su `console.log('Modelos sincronizados.')`); `start()` solo hace `sequelize.authenticate()` antes de `app.listen(...)`.
- [x] 9.6 Verificar con la stack Docker: dado que cualquier entorno ya levantado antes de este ajuste tiene tablas creadas por `sync()` sin `SequelizeMeta`, ejecutar `docker compose down -v` (recrea el volumen `db_data` desde cero) y luego `docker compose up --build`; confirmar en los logs de `backend` que `sequelize-cli db:migrate` corre automáticamente (por el script `dev`) y reporta las 3 migraciones como aplicadas (`== <timestamp>-create-users: migrating/migrated ==`, ídem para las otras dos), y que el backend arranca sin llamar a `sync()`. Confirmar con `docker compose exec db psql -U postgres -d prueba_smk -c "\dt"` que existen `users`, `documents`, `document_rows` y `SequelizeMeta`.
- [x] 9.7 Repetir el flujo de verificación de las secciones 1 a 8 (registro, login, carga de CSV, soft delete, etc.) contra el esquema creado por migraciones, para confirmar que no hay diferencias de comportamiento respecto al esquema que creaba `sync()`.

## 10. Documentación

- [x] 10.1 En `README.md`, tabla "Variables de entorno (`backend/.env`)": agregar filas para `POSTGRES_DB`/`POSTGRES_USER`/`POSTGRES_PASSWORD` (nota: deben coincidir con `DB_NAME`/`DB_USER`/`DB_PASSWORD`, las usa el contenedor de Postgres) y `MAX_CSV_FILE_SIZE_MB` (límite de tamaño de CSV, default `5`).
- [x] 10.2 En `README.md`, sección "API principal": actualizar la fila de `POST /api/documents` y `GET /api/documents` si mencionan nombres de campo de la respuesta (revisar el texto actual; si no menciona claves JSON explícitas, no hace falta editar esa tabla). Agregar una nota breve en la sección de RBAC o en una nueva línea aclarando que "Eliminar un documento" es un borrado lógico (el archivo original se conserva en el servidor).
- [x] 10.3 Confirmar que `README.md` no contradice el nuevo comportamiento de la tarea 2 (registro): la sección "Cómo levantar el proyecto" ya dice "el registro público solo crea usuarios con `rol: 'user'`"; añadir una frase indicando que el formulario ahora muestra un selector de rol pero solo permite elegir "Usuario" desde la UI.
- [x] 10.4 En `README.md`: documentar el nuevo mecanismo de migraciones (directorio `backend/src/database/migrations/`, comando `npm run migrate` — ya integrado en `start`/`dev` —, y la nota de transición: cualquier entorno levantado antes de este ajuste necesita `docker compose down -v` una sola vez).

## 11. Verificación final integrada

- [x] 11.1 Con la stack completa levantada (`docker compose down -v && docker compose up --build`), ejecutar el flujo end-to-end: crear `backend/.env` y `frontend/.env` desde los `.example` actualizados, registrar un usuario normal, promoverlo a admin con `npm run seed:admin`, iniciar sesión como admin, subir un CSV válido, verlo en la tabla, descargarlo, eliminarlo (soft delete) y confirmar que desaparece de la tabla pero el archivo permanece en `backend/uploads/` y las filas permanecen en `document_rows`.
- [x] 11.2 Repetir el intento de registro con `rol: "admin"` vía API directa y confirmar 400 (ya cubierto en 2.5, repetir aquí como parte de la verificación integrada final).
- [x] 11.3 Revisar que no queden identificadores en español fuera de las excepciones documentadas (CSV: `correo/nombre/telefono/ciudad/notas`; auth wire: `nombre/contraseña/confirmarContraseña/rol`) haciendo una búsqueda de texto en `backend/src` y `frontend/src` por los términos ya identificados en este change (`usuarioId`, `nombreOriginal`, `nombreArchivo`, `rutaArchivo`, `numeroRegistros`, `validarFilas`, `rolesPermitidos`, `documentos` como nombre de variable/prop, `errorCarga`, `nombreUsuario`, `formatFecha`, `nombreDescarga`, `eliminarArchivoSiExiste`) y confirmar que ya no aparecen (salvo, si corresponde, en comentarios o en el historial de `openspec/changes/archive/`, que no se tocan).
- [x] 11.4 Confirmar en los logs de `backend` (`docker compose up --build` desde volumen limpio) que `sequelize-cli db:migrate` corre y aplica las 3 migraciones antes de "Servidor escuchando en el puerto 3000", y que ya no aparece el log "Modelos sincronizados." (eliminado junto con `sync()`).
