...
- [x] 4.2 Verificación end-to-end: con `docker compose up` (db+backend+frontend reales), probado con curl contra la API real: registro admin (201), registro user (201), registro con contraseñas distintas (400 con detalle), registro de nombre duplicado (409), login admin y login user (200 + JWT válido), `GET /api/documents` sin token (401), `GET /api/documents` con token (200). Todos los códigos de estado y shapes de respuesta coinciden exactamente con la spec.
