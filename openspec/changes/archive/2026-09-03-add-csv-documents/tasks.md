...
- [x] 5.2 Verificación end-to-end (con `docker compose up` real): carga válida (201, `numeroRegistros: 2` correcto), carga con fila inválida (400 con detalle exacto fila/campo/mensaje, nada persistido), `GET /api/documents` refleja lo cargado con usuario/fecha/registros correctos, descarga devuelve el CSV original byte a byte, `DELETE` con `user` (403), `DELETE` con `admin` (200) y el documento desaparece del listado subsecuente. Todo verificado con curl contra la API real.
